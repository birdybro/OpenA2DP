...
- **C** for first-party code; **C++** only behind `extern "C"` for COM and ImGui backends.
...
Each binary embeds a `VERSIONINFO` + `IDI_APP_ICON` resource via `scripts/version_{gui,cli}.rc`. Bump `OA2DP_VER_*` in BOTH .rc files and add a `CHANGELOG.md` entry on every release.

### CI / releases

`.github/workflows/build.yml` runs on push, PR, and `v*.*.*` tags. Uses `actions/checkout@v4` (recursive submodules), `ilammy/msvc-dev-cmd@v1`, then `scripts\build.bat`. Tag pushes zip both binaries and create a GitHub Release via `softprops/action-gh-release@v2`.
...
- **Cross-thread COM**: `audio_status.cpp` does NOT cache an enumerator. Each call creates and releases its own. Workers use `oa2dp_audio_status_thread_init`/`_shutdown` for per-thread COM apartments.
...
- **Stats** (`stats.c`): in-memory `InterlockedIncrement` counters for reconnects, heal triggers/recoveries/failures, HFP watchdog, stack switches. Per-session, not persisted. Footer in device list panel.
- **History** (`history.c`): every connect/disconnect transition appended to `%APPDATA%\OpenA2DP\<addr>.history`. Auto-prunes oldest half over 200 lines. Status panel shows last 12.
- **Window state**: `oa2dp_window_state_save`/`_load` in `core/config.c` persist via `GetWindowPlacement` (captures the normal rect even when minimized to tray). Saved on shutdown, loaded before `CreateWindowW`.
