...
- **Approachable by default** — On first launch, only the things you actually need are visible: device list, Reconnect, Reset, a minimal status readout (connection state, battery, the connect-but-silent warning), and a real-time audio visualizer. The "Advanced Mode" checkbox in the top-right unlocks codec editing, A2DP stack control, service toggles, watchdogs, capabilities, history, and the diagnostic log.
- **Audio visualizer** — Small spectrum visualizer in the bottom-left corner driven by WASAPI loopback capture of the system default render endpoint. 16 log-spaced frequency bands computed via Goertzel filter on a background thread, peak-hold + decay smoothed, color-graded cyan → green → yellow → red. Visible in both simple and advanced mode, works even with no headphones connected.
- **Reset All Settings to Defaults** — Escape hatch at the bottom of the device list. Confirm modal, then resets every device profile to its built-in defaults and snaps the main window back to its default size.
- **Live window resize** — Window contents reflow in real time during a border drag instead of staying frozen until you let go of the mouse.
